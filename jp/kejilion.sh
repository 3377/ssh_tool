...
ldnmp_web_status() {
	...
	while true; do
		...
		echo "站点目录"
		echo "------------------------"
		echo -e "数据 ${gl_hui}/home/web/html${gl_bai}証明書${gl_hui}/home/web/certs${gl_bai}構成${gl_hui}/home/web/conf.d${gl_bai}"
		echo "------------------------"
		echo ""
		echo "操作"
		echo "------------------------"
	...
	while true; do
		...
		case "$Limiting" in
			...
			5)  # 删除目录
				...
				rm -rf "$dirname" && echo "目录已删除" || echo "削除に失敗しました"
				send_stats "删除目录"
				;;
			6)  # 返回上一级选单目录
				cd ..
			...
			15) # 删除文件
				...
				rm -f "$filename" && echo "ファイルが削除されました" || echo "删除失败"
				send_stats "删除文件"
				;;
			21) # 压缩文件/目录
