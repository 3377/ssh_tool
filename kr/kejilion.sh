	...
	while true; do
		...
		case "$Limiting" in
			...
			15) # 删除文件
				...
				rm -f "$filename" && echo "文件已删除" || echo "삭제 실패"
				send_stats "删除文件"
				;;
			21) # 压缩文件/目录
			...
			22) # 解压文件/目录
				...
				install tar
				tar -xzvf "$filename" && echo "已解压 $filename" || echo "압축 해제 실패"
				send_stats "解压文件/目录"
				;;
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}安装科技lion脚本         ${gl_kjlan}12. ${gl_bai}시스템 업데이트${gl_kjlan}13. ${gl_bai}清理系统"
	  ...
	  case $sub_choice in
		  ...
		  5)
			  ...
			  echo -e "请上传您的 ${gl_huang}servers.py${gl_bai}파일을 제출하다${gl_huang}/root/cluster/${gl_bai} 完成还原！"
			  break_end
			  ;;
...
echo -e "기술 사자 주변기기"
echo "------------------------"
echo -e "${gl_kjlan}B站: ${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}오일 파이프:${gl_bai}https://www.youtube.com/@kejilion${gl_bai}"
...
echo "패키지 제거 k 제거 nano wget | k 델 나노 wget | k uninstall nano wget | k uninstall nano wget"
...
echo "Software startup k start sshd | k sshd를 시작하세요"
echo "소프트웨어 중지 k 중지 sshd | k stop sshd"
